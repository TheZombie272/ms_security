    ...
    private ValidatorsService theValidatorsService;
        ...
        public HashMap<String, Object> login(@RequestBody User theNewUser,
            ...
            if (theActualUser != null
                ...
                // Generar token JWT inmediatamente (sin 2FA)
                String token = this.theJwtService.generateToken(theActualUser);

                // crear sesion y guardar el token JWT en la sesión
                Session theUserSession = new Session();
                theUserSession.setUser(theActualUser);
                theUserSession.setToken(token);
                theUserSession.setType("session");
                Session theSendSession = theSessionRepository.save(theUserSession);

                // No devolvemos el token dentro del objeto session por seguridad
        ...
        // Verificación usada para negocio
